...
<head>
    ...
    <style>
        ...
        @media (min-width: 601px) {
            ...
            .sheep-img { position: fixed; bottom: 68px; left: 50%; transform: translateX(-50%); width: min(80vh, 700px); z-index: 0; pointer-events: none; }
            .cloud-wrap { position: fixed; top: 40px; width: 520px; height: 330px; z-index: 10; }
            .cloud-inner { padding: 0 50px 40px 100px; font-size: 15px; }
            .collection-btn { position: fixed; z-index: 10; }
        }
        @media (max-width: 600px) {
            .pc-only { display: none !important; }
            .sp-sheep { position: fixed; bottom: 0px; left: 85%; transform: translateX(-50%); width: 1300px; z-index: 1; pointer-events: none; max-width: none; height: 550px; object-fit: cover; }
            .sp-cloud { position: fixed; top: 0; left: 50%; transform: translateX(-50%); width: 130vw; height: 82vw; z-index: 10; }
            .sp-cloud-inner { padding: 6px 30px 28px 28px; font-size: 13px; line-height: 1.6; }
            .sp-btn { position: fixed; top: 80vw; right: 12px; z-index: 10; }
        }
    </style>
</head>
<body class="flex justify-center items-start min-h-screen">
    ...
    <div class="pc-only w-full max-w-[390px] min-h-screen bg-[#fde8e8] relative">
        <div class="cloud-wrap" id="cloud-wrap">
            <div class="w-full h-full bg-contain bg-no-repeat bg-center flex items-start justify-center pt-[10%] pb-[22%]" style="background-image: url('{{ asset('images/cloud.png') }}');">
        ...
        <div class="collection-btn" id="collection-btn">
            <button onclick="openPopup()" class="bg-[#efe8cf] border border-[#b6aa83] rounded-2xl px-6 py-3 text-[#4a3f3f] text-sm font-bold shadow-sm active:scale-95 transition">コレクション<br>ショップ</button>
        </div>
        <div class="fixed top-4 right-4 z-10 bg-[#efe8cf] border border-[#b6aa83] rounded-full px-4 py-2 text-[#4a3f3f] text-sm font-bold shadow-sm flex items-center gap-1">
            <span class="text-yellow-500">⭐</span> <span class="total-points-display">0</span> pt
        </div>
    ...
    <div class="sp-only">
        ...
        <div class="sp-cloud">
            <div class="w-full h-full bg-contain bg-no-repeat bg-center flex items-start justify-center pt-[10%] pb-[28%]" style="background-image: url('{{ asset('images/cloud.png') }}');">
        ...
        <div class="sp-btn">
            <button onclick="openPopup()" class="bg-[#efe8cf] border border-[#b6aa83] rounded-2xl px-4 py-2 text-[#4a3f3f] text-xs font-bold shadow-sm active:scale-95 transition">コレクション<br>ショップ</button>
        </div>
        <div class="fixed top-4 right-4 z-10 bg-[#efe8cf] border border-[#b6aa83] rounded-full px-5 py-2 text-[#4a3f3f] text-base font-bold shadow-sm flex items-center gap-1">
            <span class="text-yellow-500">⭐</span> <span class="total-points-display">0</span> pt
        </div>
    </div>
    ...
    <div id="popup" class="hidden fixed inset-0 z-50" onclick="closePopup()">
        <div id="popup-bubble" class="absolute flex flex-col items-center" style="bottom: calc(68px + 48%); left: 50%; transform: translateX(-50%);" onclick="event.stopPropagation()">
            <div id="popup-cloud" class="relative flex items-center justify-center bg-contain bg-no-repeat bg-center" style="background-image: url('{{ asset('images/cloud.png') }}');">
                <div class="flex flex-col gap-3 mt-[-40px]">
                    <a href="/collection" class="bg-[#fff9e6] rounded-full py-3 px-10 text-[#4a3f3f] font-bold shadow-sm border border-[#e8dfc5] text-center text-sm">コレクション画面</a>
                    <a href="/shop" class="bg-[#fff9e6] rounded-full py-3 px-10 text-[#4a3f3f] font-bold shadow-sm border border-[#e8dfc5] text-center text-sm">ショップ画面</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        (function() {
            ...
            if (item && map[item]) {
                ...
                if(document.getElementById('pc-sheep')) document.getElementById('pc-sheep').src = src;
                if(document.getElementById('sp-sheep')) document.getElementById('sp-sheep').src = src;
            }
        })();
    </script>
</body>
